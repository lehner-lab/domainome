...
ggplot(mutated_domainome_invitro_ddGs[dom_ID %in% ivt_comparison_summary_dominfo$dom_ID & mean_count>5,])+
  ...
  theme_classic()
ggsave("output_files/ED_Figure_1c_correlations_to_invitro.pdf",height=5,width=6)
...
ggplot(aPCA_rocklin_merged_ddGs[dom_ID %in% cors_to_rocklin$dom_ID & mean_count>5,])+
  ...
  theme_classic()
ggsave("output_files/ED_Figure_1d_correlations_to_rocklin.pdf",height=5,width = 7)
...
ivt_comparison_summary_dominfo[,fraction_explainable_variance:=cors_to_ddG_individualreps**2/cors_replicates_individualreps**2]
cors_to_rocklin[,fraction_explainable_variance:=ddG_r_mean**2/cors_nostops**2]


fev<-data.frame(fraction_explainable_variance=c(ivt_comparison_summary_dominfo$fraction_explainable_variance,cors_to_rocklin$fraction_explainable_variance),ref_data=c(rep("in vitro",nrow(ivt_comparison_summary_dominfo)),rep("rocklin",nrow(cors_to_rocklin))))

ggplot(fev)+
  geom_boxplot(aes(y=fraction_explainable_variance,x=ref_data))+
  geom_jitter(aes(y=fraction_explainable_variance,x=ref_data))+
  coord_cartesian(ylim=c(0,1))+
  theme_classic()+
  ylab("fraction of explainable variance")
ggsave("output_files/Figure_1g_fraction_explainable_variance.pdf",width=3,height = 4)
...
median(ivt_comparison_summary_dominfo$fraction_explainable_variance)
median(cors_to_rocklin$fraction_explainable_variance)
